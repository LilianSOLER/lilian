...
$podcasts = slice_podcast_by(sort_podcasts_by_mktime(get_podcasts($METH_SCI['url'])), "week");

function week_podcasts_to_HTML_table($podcasts, $week){
  $table = "<h2>Podcasts de la semaine " . $week . " " . date('Y', $podcasts[0]["pubDate"]) . " :</h2>";
  $table .= "<table>";
  $table .= "<tr>";
...
{
  $res = "";
  foreach($podcasts as $week => $week_podcasts){
    $res .= week_podcasts_to_HTML_table($week_podcasts, $week);
  }
  return $res;
}
?>
...
<head>
  ...
  <title>2. Intercalaire hebdomadaire</title>
</head>

<body>
  <h1>2. Intercalaire hebdomadaire</h1>
