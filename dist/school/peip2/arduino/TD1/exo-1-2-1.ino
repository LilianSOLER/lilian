...
void loop() {
  ...
  if (tension_5 > float(4)){Serial.println("Entrée dans le second if");
  digitalWrite(LV,0);Serial.println("Led vert allumé"); 
  digitalWrite(LO,0);Serial.println("Led Orange allumé");
  digitalWrite(LR,0);Serial.println("Led Rouge allumé");}

  if ((tension_5 <= float(4))&&(tension_5 > float(3))) {
