...
const queue_js_1 = require("./types/queue.js");
class Typed {
    constructor(options) {
        this._resetter = new resetter_js_1.Resetter();
        this._randomChars = new random_char_js_1.RandomChars();
        this._typeQueue = new queue_js_1.Queue(this._resetter);
        this._ffQueue = new queue_js_1.Queue(this._resetter);
    ...
    async typeLetter() {
        ...
        if (queue === this._queue) {
            ...
            this.updateText();
            await (0, wait_js_1.wait)(this.options.perLetterDelay, this._resetter);
        }
    ...
    async typeBackspace() {
        ...
        if (currentBackspaceItem.length > 0) {
            this.deleteLetter();
            this.updateText();
            await (0, wait_js_1.wait)(this.options.eraseDelay, this._resetter);
        }
    ...
    async shouldError(currentWrongLettersCount, intendedChar, wasFF, nearbyChar) {
        ...
        if (this.options.noSpecialCharErrors && (0, is_special_char_js_1.isSpecialChar)(intendedChar)) {
            willError = false;
        }
        if (!nearbyChar) {
            willError = false;
        ...
        if (!willError || !nearbyChar) {
            if (currentWrongLettersCount > 0) {
                if (!this._fastForward || wasFF) {
                    await (0, wait_js_1.wait)(this.options.errorDelay, this._resetter);
                }
            }
        }
        return willError;
